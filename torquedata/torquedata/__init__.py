...
def permissions_sha(sheet_name, wiki_key, group):
    ...
    return hashlib.sha224(
        ...
        str(permissions[sheet_name][wiki_key]
            [group]["columns"]).encode('utf-8')
    ).hexdigest()
...
def index_search(group, sheet_name, wiki_key):
    ...
    dir = os.path.join(
        app.config['SPREADSHEET_FOLDER'], sheet_name, "indices", sha)

    if(index.exists_in(dir)):
        print("Index already exists for " + sheet_name + " / " +
              wiki_key + " / " + group + " (or comparable)")
    ...
    for o in cull_invalid_objects(group, sheet_name, wiki_key):
        writer.add_document(
            ...
            content=" ".join([str(c) for c in cull_invalid_columns(
                o, permissions[sheet_name][wiki_key][group]["columns"]).values()])
        )
    writer.commit()
...
def load_sheet(sheet_name):
    ...
    edits[sheet_name] = {}
    reader = csv.reader(
        open(os.path.join(app.config.get("SPREADSHEET_FOLDER"),
                          sheet_name, sheet_name + ".csv"), encoding='utf-8'),
        delimiter=',',
        quotechar='"'
    )
